                    ...
                    <div class="form-wrap">
                        ...
                        <form action="" method="POST">
                            ...
                            <div class="input">
                                ...
                                <input type="date" name="bdate" id="bdate" placeholder="Please input your birth date..." required> 
                            </div>
                            <div class="input">
                        ...
                        <?php
                            ...
                            if(isset($_POST['submit'])){
                                ...
                                $rentprice = null;
                                $rentavail = null;
                                $result = mysqli_query($conn, "SELECT MotorID FROM tbmotorcycle WHERE MotorID = '$motorid'");
                                $row = mysqli_fetch_array($result);
                                if ($row > 0){
                                    $idvalid = true;
                                    $mtr_sql = "SELECT MotorID, MotorName, MotorPlat, Price, RentStat FROM tbmotorcycle WHERE MotorID = '$motorid'";
                                    $result= mysqli_query($conn, $mtr_sql);
                                    $row = mysqli_fetch_array($result);
                                    $motorname = $row[1];
                                    $motorplat = $row[2];
                                    $price = $row[3];
                                    ...
                                    if ($rentavail == 'AVAILABLE'){
                                        ...
                                        echo '<script>';
                                        echo 'alert("Motorcycle is not available!")';
                                        echo '</script>';
                                    }
                                }
                                else{
                                    echo '<script>';
                                    echo 'alert("Motorcycle ID not founded!")';
                                    echo '</script>';
                                }
                                ...
                                if(!empty($name) && !empty($gender) && !empty($bdate) && !empty($address) && !empty($phone) && !empty($motorid) && !empty($rentdate) && !empty($returndate) && $idvalid && $rentvalid && $avail){
                                    ...
                                    $order_sql = "INSERT INTO tborder VALUES (NULL, '$name','$phone','$motorid','$motorname','$motorplat','$rentdate','$returndate','$rentprice')";
                                    $motor_sql = "UPDATE tbmotorcycle SET RentStat = 'NOT AVAILABLE' WHERE MotorID = '$motorid'";
                                    if ($nodupephone){
